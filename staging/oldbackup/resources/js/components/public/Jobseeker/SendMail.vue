<template>
  <div>
    ...
    <section class="my-send-mail mt-3 mb-3">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <div class="mail-box">
                ...
                <div class="inbox-body mail-box-section">
                  <table class="table table-inbox table-bordered table-hover">
                    <tbody>
                      ...
                      <tr
                        v-for="sendMailList in sendMailLists"
                        :key="sendMailList.id"
                      >
                        ...
                        <td
                          class="view-message dont-show inbox-small-cells"
                          width="5%"
                        >
                          {{
                        ...
                        <td class="view-message">
                          <router-link
                            :to="`/inbox-single/${sendMailList.id}`"
                            >{{ sendMailList.subject }}</router-link
                          >
                        </td>
                        <td
                          class="view-message inbox-small-cells"
                          width="5%"
                          v-if="sendMailList.attachment"
                        >
                        ...
                        <td class="view-message text-right" width="14%">
                          {{ sendMailList.created_at | checkCurrentDateTime }}
                        </td>
                      </tr>
                    </tbody>
                  </table>
...
export default {
  ...
  mounted() {
    ...
    axios
      .get("get-send-mail-lists")
      .then((response) => {
        this.sendMailLists = response.data.data;
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  methods: {
    ...
    sendMessage() {
      this.status = true;
      axios
        .post("/send-message-to-employer", this.form)
        .then((response) => {
