        ...
        <div class="col-md-12">
          <div class="card">
            ...
            <div class="card-header">
              ...
              <a href="" @click.prevent="exportemployers()" type="button"
              ...
              <button class="btn btn-primary gr">
                <router-link to="/add-employer" style="color:#fff"> Add Employer</router-link>
              </button>
            </div>
            <div class="card-header">
              <div class="row">
            ...
            <div class="card-body">
              <table id="example1"
                ...
                <tbody>
                <tr v-for="(employers,index) in myData.data" :key="employers.id">
                  <td><input type="checkbox" name="emp_check" :value="employers.id"
                             @click="uncheck(employers.id)" v-model="checkedNames"></td>
                  <td>{{ index + 1 }}</td>
...
<!--                  <td><input @click.prevent="disableedit(employers.id)"-->
<!--                             name="can_edit_company" type="checkbox"-->
<!--                             v-if="employers.can_edit_company==='Yes'" checked>-->
<!--                    <input @click.prevent="enableedit(employers.id)"-->
<!--                           name="can_edit_company" type="checkbox"-->
<!--                           v-if="employers.can_edit_company==='No'">-->
<!--                  </td>-->
                  <td>{{ employers.active }}</td>

                  <td>

                    <router-link :to="`/edit-employer/${employers.id}`"><i
                        class="far fa-edit"></i></router-link>
                    <!-- <a href="" @click.prevent = "deleteEmployer(employers.id)"><i class="far fa-trash-alt"></i></a> -->
                    <a href="" @click.prevent="deactiveemployer(employers.id)"
                    ...
                    <a href="" @click.prevent="activeemployer(employers.id)"
...
export default {
  ...
  methods: {
    ...
    getFilterData: function (page = 1) {
      let self = this;
      axios.get('employer-filter?industry_id=' + this.industry_id + '&company_id=' + this.company_id + '&email=' + this.email + '&status=' + this.status + '&page=' + page).then(function (response) {
    ...
    exportcompanieschecked() {
      let chk_val = document.getElementById("checkvalue").value;
      var uri = '/export-employer-checked/' + chk_val;
      swal.fire({
        title: 'Are you sure want export all data?',
        text: "Companies details!",
        type: 'warning',
        ...
        if (result.value) {
          ...
          window.open('/export-employer-checked/' + chk_val, '_blank');
        }
      });
    }
